...
export class CommandsService {
  ...
  async sendIgnitionCommand(userId: string, deviceId: string, state: boolean): Promise<CommandLog> {
    ...
    try {
      const result = await this.traccarService.sendCommand(Number(device.traccarDeviceId), type);
  ...
  async retryCommand(logId: string) {
    ...
    try {
      ...
      const result = await this.traccarService.sendCommand(Number(device.traccarDeviceId), log.type);
      log.status = 'success';
      log.result = result;
    } catch (error) {
      log.status = 'failed';
