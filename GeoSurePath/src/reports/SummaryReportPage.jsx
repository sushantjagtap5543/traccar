...
import exportCsv from '../common/util/exportCsv';
...
const SummaryReportPage = () => {
  ...
  const onExport = useCatch(async (format) => {
    ...
    const fileName = daily ? 'daily_summary' : 'summary';
    const sheets = new Map([[title, rows]]);
    
    if (format === 'pdf') {
       await exportPdf(title, `${fileName}.pdf`, sheets);
    } else if (format === 'csv') {
       await exportCsv(title, `${fileName}.csv`, sheets);
    } else {
       await exportExcel(title, `${fileName}.xlsx`, sheets, theme);
    }
  });
