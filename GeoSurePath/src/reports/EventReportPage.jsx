...
const EventReportPage = () => {
  ...
  const onExport = useCatch(async (format) => {
    ...
    const title = t('reportEvents');
    if (format === 'pdf') {
       await exportPdf(title, 'events.pdf', sheets);
    } else if (format === 'csv') {
       await exportCsv(title, 'events.csv', sheets);
    } else {
       await exportExcel(title, 'events.xlsx', sheets, theme);
    }
  });
