...
export const geometryToArea = (feature) => {
  // Mapbox Draw Circle plugin outputs a center property with radiusInKm
  if (feature.properties && feature.properties.isCircle) {
    const center = feature.properties.center;
    const radiusMeters = feature.properties.radiusInKm * 1000;
    return `CIRCLE (${center[1]} ${center[0]}, ${radiusMeters})`;
  }
  
  const wkt = stringify(reverseCoordinates(feature.geometry || feature));
  
  // Traccar specifically requires Polyline instead of LineString for Routes
  if (wkt.startsWith('LINESTRING')) {
    return wkt.replace('LINESTRING', 'LINESTRING'); // Traccar supports LINESTRING
  }
  return wkt;
};
