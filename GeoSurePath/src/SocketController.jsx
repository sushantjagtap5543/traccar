...
const SocketController = () => {
    ...
    (events) => {
      ...
      setNotifications(
        events.map((event) => {
          ...
          const vehicleName = device ? device.name : 'Unknown Vehicle';
          return {
            id: event.id,
            message: `${vehicleName}: ${event.attributes.message || event.type}`,
            show: true,
          };
        }),
      );
  ...
  const connectSocket = () => {
    ...
    let socketUrl;
    if (apiUrl) {
      socketUrl = apiUrl.replace(/^http/, 'ws') + '/api/socket';
    } else {
